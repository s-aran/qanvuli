...
CLI、ターミナル UI、Rust API、MCP サーバーを備えています。各データソースの情報を取り込んだ後は、ネットワークへ問い合わせることなく、ローカル環境で検索できます。
...
`--eager-cleanup`（`-C`）を指定すると、自動ダウンロードした CVE と OSV のアーカイブを、それぞれの投入処理が成功した時点で削除します。これにより、後続の初期化処理におけるピーク時のディスク使用量を抑えられます。CWE と CAPEC の自動ダウンロードファイルは、従来どおり投入直後に削除されます。`--zip` で指定したユーザー所有のアーカイブは削除しません。`--eager-cleanup` と `--keep` は同時に指定できません。
...
リモートデータを対象とする `update` は再開できますが、すべてのデータソースをまとめた単一のアトミックなトランザクションではありません。後続の CWE、CAPEC、OSV、KEV、EPSS の更新に失敗しても、適用済みの CVE 差分とカーソルは保持されます。`qanvuli update` を再実行すれば、保存済みの状態から処理を再開できます。そのため、更新コマンドが失敗しても、データがまったく変更されていないとは限りません。
...
スキャン結果の JSON は標準出力に出力します。`--sarif-output <PATH>` を指定すると、同じ結果を、脆弱性レポートやコードスキャンとの連携に利用できる SARIF 2.1.0 ファイルにも出力します。
...
MCP の検索では、遅いクエリによって同時実行中のツール呼び出しが待たされないよう、既定で 4 本の独立した SQLite 読み取り接続を使用します。メモリ容量とストレージ性能に合わせて調整する場合は、`QANVULI_MCP_READ_CONNECTIONS` に 1 から 8 までの値を指定してください。データベース更新は引き続き排他的に実行します。

...
MCP の検索では、遅いクエリによって同時実行中のツール呼び出しが待たされないよう、既定で4本の独立した SQLite 読み取り接続を使用します。メモリ容量とストレージ性能に合わせて調整する場合は、`QANVULI_MCP_READ_CONNECTIONS` に1から8までの値を指定してください。データベース更新は引き続き排他的に実行します。

        ...
        Paragraph::new(app.capec_query.as_str()).block(
            Block::default()
                .title(format!(
                    ...
                    shown(&app.capec_status_filter),
                    shown(&app.capec_type_filter),
                    shown(&app.capec_cwe_filter)
                ))
                .borders(Borders::ALL),
        ),
        main[0],
...
fn draw_list(frame: &mut ratatui::Frame<'_>, app: &App, area: Rect) {
    let block = Block::default()
        .title(format!("CAPEC ({})", app.capec_results.len()))
        .borders(Borders::ALL)
        .border_style(focus_style(app.focus == PaneFocus::Left));
    let inner = block.inner(area);
    frame.render_widget(block, area);
    let start = (app.capec_scroll as usize).min(app.capec_results.len());
    let lines = app
        .capec_results
        .iter()
        .skip(start)
        ...
        .enumerate()
        .map(|(offset, capec)| {
            let style = if start + offset == app.capec_selected {
...
fn draw_detail(frame: &mut ratatui::Frame<'_>, app: &App, search: &DetailSearch, area: Rect) {
    ...
    frame.render_widget(
        Paragraph::new(lines)
            ...
            .scroll((app.capec_detail_scroll, 0))
            .wrap(Wrap { trim: true }),
        area,
    );
}
...
fn draw_filter(frame: &mut ratatui::Frame<'_>, app: &App) {
    ...
    let values = [
        ("Status", &app.capec_status_filter),
        ("Type", &app.capec_type_filter),
        ("CWE ID", &app.capec_cwe_filter),
    ];
    let lines = values
        .iter()
        .enumerate()
        .map(|(index, (label, value))| {
            Line::from(Span::styled(
                format!("{label}: {value}"),
                if index == app.capec_filter_field {
                    Style::default().add_modifier(Modifier::REVERSED)
                } else {
                    Style::default()
                },
...
fn draw_taxonomy(frame: &mut ratatui::Frame<'_>, app: &App) {
    ...
    frame.render_widget(Clear, area);
    let tab = ["Category", "View"][app.capec_taxonomy_tab];
    let section = ["Overview", "Members", "Sources", "History"][app.capec_taxonomy_section];
    let mut lines = vec![Line::from(format!(
        "{tab} | {section}   Tab switch classification   ←/→ switch section"
    ))];
    lines.push(Line::from(""));
    if let Some(detail) = &app.capec_taxonomy {
        if app.capec_taxonomy_tab == 0 {
            if let Some(category) = detail.categories.get(app.capec_taxonomy_selected) {
                lines.push(Line::from(format!(
                    "Category-{} [{}] {}",
                    category.category.id, category.category.status, category.category.name
                )));
                append_category_section(&mut lines, category, app.capec_taxonomy_section);
            } else {
                lines.push(Line::from("No related Category"));
            }
        } else if let Some(view) = detail.views.get(app.capec_taxonomy_selected) {
            lines.push(Line::from(format!(
                "View-{} [{}] {}",
                view.view.id, view.view.status, view.view.name
            )));
            append_view_section(&mut lines, view, app.capec_taxonomy_section);
        } else {
    ...
    frame.render_widget(
        Paragraph::new(lines)
            ...
            .scroll((app.capec_taxonomy_scroll, 0))
            .wrap(Wrap { trim: true }),
        area,
    );
}

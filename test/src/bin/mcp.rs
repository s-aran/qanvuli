...
fn main() {
    ...
    loop {
        let Some(message) = read_message(&mut input).expect("failed to read MCP message") else {
            break;
        };
        ...
        let request = match serde_json::from_slice::<JsonRpcRequest>(&message) {
            ...
            Err(err) => {
                ...
                write_message(&mut output, &response).expect("failed to write MCP response");
                continue;
            }
        };
...
fn database_url() -> String {
    std::env::var("QANVULI_DB_URL").unwrap_or_else(|_| {
        let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("db.sqlite");
...
async fn call_tool(db: &CveDatabase, params: Value) -> Result<Value, String> {
    ...
    let value = match name {
        "search_by_cwe" => {
            ...
            let cves = db
                .search_cves_by_cwe(&args.cwe_ids, limit(args.limit), offset(args.offset))
                .await
        ...
        "search_text" => {
            ...
            let cves = db
                .search_cves_by_text(&args.query, limit(args.limit), offset(args.offset))
                .await
                .map_err(|err| err.to_string())?;
            json!(summaries(cves))
        }
...
fn summaries(cves: Vec<cve::Model>) -> Vec<Value> {
    cves.into_iter().map(summary).collect()
}

fn summary(cve: cve::Model) -> Value {
    json!({
        "cve_id": cve.cve_id,
        "state": cve.state,
